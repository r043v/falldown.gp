...
void load_anim(struct gfx * gfx,char id_anim,unsigned char frm_start,unsigned char frm_number,unsigned char * anim,unsigned short time, char repeat,char sens, unsigned short sx,unsigned short sy,unsigned short px,unsigned short py,unsigned short tr_color)
{
	...
	for(cpt=0;cpt<frm_number;cpt++)
	{
		...
		if(tr_color)
		{
			...
			do	// bound up
			{
				for(cpt_x=0;cpt_x<gfx->size_x-1;cpt_x++)
				{
					...
					{
						transp_ = 0 ;
						gfx->real_pos_y[frm_start + cpt] = cpt_y ;
						cpt_x = gfx->size_x ;
					}
				} ;
			...
			if(!Atransp_)
				do	// bound left
				{
					for(cpt_y=0;cpt_y<gfx->size_y-1;cpt_y++)
					{
						unsigned short color ;
			...
			if(!Atransp_)
				do	// bound right
				{
					for(cpt_y=0;cpt_y<gfx->size_y;cpt_y++)
					{
						...
						{
							transp_ = 0 ;
							gfx->real_size_x[frm_start + cpt] = cpt_x - gfx->real_pos_x[frm_start + cpt] ;
						}
					} ;
...
{
	//char buffer[20] ;
 	//GPSPRINTF(buffer,"%i|%i|%i|%i|%i|%i",*curent_frame, frame_start, frame_stop, repeat, *sens, *finish) ;
	//if(bbp==16) GpTextOut16(NULL, &gpDraw[nflip],10,25,buffer,0xB3);
	//else GpTextOut(NULL, &gpDraw[nflip],10,25,buffer,0xB3);
